...
class SharedReplayBuffer(object):
    ...
    def reset_after_update(self):
        self.rnn_states[0].fill(0)
        self.rnn_states_critic[0].fill(0)
        self.masks[0].fill(1)
        self.bad_masks[0].fill(1)
        self.step = 0
    ...
    def partial_naive_recurrent_generator(self, advantages, idx, num_mini_batch):
        """
        Yield training data for non-chunked RNN training.
        :param advantages: (np.ndarray) advantage estimates.
        :param num_mini_batch: (int) number of minibatches to split the batch into.
        ...
        share_obs = self.share_obs[:, :, idx].reshape(-1, batch_size, *self.share_obs.shape[3:])
        obs = self.obs[:, :, idx].reshape(-1, batch_size, *self.obs.shape[3:])
        rnn_states = self.rnn_states[:, :, idx].reshape(-1, batch_size, *self.rnn_states.shape[3:])
        rnn_states_critic = self.rnn_states_critic[:, :, idx].reshape(-1, batch_size, *self.rnn_states_critic.shape[3:])
        actions = self.actions[:, :, idx].reshape(-1, batch_size, self.actions.shape[-1])
        if self.available_actions is not None:
            available_actions = self.available_actions[:, :, idx].reshape(-1, batch_size, self.available_actions.shape[-1])
        value_preds = self.value_preds[:, :, idx].reshape(-1, batch_size, 1)
        returns = self.returns[:, :, idx].reshape(-1, batch_size, 1)
        masks = self.masks[:, :, idx].reshape(-1, batch_size, 1)
        active_masks = self.active_masks[:, :, idx].reshape(-1, batch_size, 1)
        action_log_probs = self.action_log_probs[:, :, idx].reshape(-1, batch_size, self.action_log_probs.shape[-1])
        advantages = advantages[:, :, idx].reshape(-1, batch_size, 1)
    ...
    def partial_recurrent_generator(self, advantages, idx, num_mini_batch, data_chunk_length):
        """
        Yield training data for chunked RNN training.
        :param advantages: (np.ndarray) advantage estimates.
        :param num_mini_batch: (int) number of minibatches to split the batch into.
        ...
        if len(self.share_obs.shape) > 4:
            share_obs = self.share_obs[:-1, :, idx].transpose(1, 2, 0, 3, 4, 5).reshape(-1, *self.share_obs.shape[3:])
            obs = self.obs[:-1, :, :, idx].transpose(1, 2, 0, 3, 4, 5).reshape(-1, *self.obs.shape[3:])
        else:
            share_obs = _cast(self.share_obs[:-1, :, idx])
            obs = _cast(self.obs[:-1, :, idx])

        actions = _cast(self.actions)[:, :, idx]
        action_log_probs = _cast(self.action_log_probs)[:, :, idx]
        advantages = _cast(advantages)[:, :, idx]
        value_preds = _cast(self.value_preds[:-1])[:, :, idx]
        returns = _cast(self.returns[:-1])[:, :, idx]
        masks = _cast(self.masks[:-1])[:, :, idx]
        active_masks = _cast(self.active_masks[:-1])[:, :, idx]
        # rnn_states = _cast(self.rnn_states[:-1])
        # rnn_states_critic = _cast(self.rnn_states_critic[:-1])
        rnn_states = self.rnn_states[:-1, :, idx].transpose(1, 2, 0, 3, 4).reshape(-1, *self.rnn_states.shape[3:])
        rnn_states_critic = self.rnn_states_critic[:-1, :, idx].transpose(1, 2, 0, 3, 4).reshape(-1,
                                                                                         *self.rnn_states_critic.shape[
                                                                                          3:])

        if self.available_actions is not None:
            available_actions = _cast(self.available_actions[:-1, :, idx])
