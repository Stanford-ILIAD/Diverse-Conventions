...
class XMAPPO():
    ...
    def ppo_update(self, sample, multisample, update_actor=True, return_aug = 1):
        """
        ...
        return_batch = return_aug * check(return_batch).to(**self.tpdv)
    ...
    def train(self, buffers, xp_buffers, xp_weight, update_actor=True):
        """
        ...
        full_xadvantages = []
        for i in range(self.popsize):
            full_xadvantages.append([])
            for j in range(self.popsize - 1):
                buffer = xp_buffers[i][j]
                if self._use_popart or self._use_valuenorm:
                    advantages = buffer.returns[:-1] - \
                        self.value_normalizer.denormalize(buffer.value_preds[:-1])
                else:
                ...
                full_xadvantages[i].append(advantages)
        ...
        for _ in range(self.ppo_epoch):
            ...
            for i in range(self.popsize):
                ...
                for j in range(self.popsize - 1):
                    if i > j:
                        xgen[i].append(xgen[j][i-1])
                        continue
                    buffer = xp_buffers[i][j]
                    advantages = full_xadvantages[i][j]
            ...
            for multisample in zip(*generators):
                ...
                for i in range(self.popsize):
                    ...
                    for j in range(self.popsize - 1):
                        if i > j:
                            xgenres[i].append(xgenres[j][i-1])
                        else:
                            xgenres[i].append(next(xgen[i][j]))
                ...
                for i in range(self.popsize):
                    ...
                    value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights \
                        = self.ppo_update(sample, multisample, update_actor)
