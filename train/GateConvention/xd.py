...
class XD:
    ...
    def gate_update(self, sample, idx):
        """
        Update actor and critic networks.
        :param sample: (Tuple) contains data batch with which to update networks.
        :update_actor: (bool) whether to update actor network.
        ...
        actions_batch = torch.ones_like(actions_batch) * idx
        ...
        # Reshape to do in a single forward pass for all steps
        action_log_probs, dist_entropy = self.gate.evaluate_actions(
            obs_batch,
            rnn_states_batch,
            actions_batch,
            masks_batch
        ...
        l2_norms = [torch.sum(torch.square(w)) for w in self.gate.parameters()]
        # divide by 2 to cancel with gradient of square
        l2_norm = sum(l2_norms) / 2

        ent_loss = 0 * entropy
        neglogp = -log_prob
        l2_loss = 0.01 * l2_norm
        loss = neglogp + ent_loss + l2_loss
    ...
    def gate_train_step(self, data_generator, train_info, idx):
        for sample in data_generator:
            (
                neglogp,
                entropy,
                ...
                l2_norm,
                l2_loss
            ), actor_loss = self.gate_update(sample, idx)

            # train_info["value_loss"] += value_loss.item()
            train_info["g_neglogp"] += neglogp.item() / len(self.agent_set)
            train_info["g_loss"] += actor_loss.item() / len(self.agent_set)
            train_info["g_entropy"] += entropy.item() / len(self.agent_set)
            train_info["g_ent_loss"] += ent_loss.item() / len(self.agent_set)
            train_info["g_prob_true_act"] += prob_true_act.item() / len(self.agent_set)
            train_info["g_l2_norm"] += l2_norm.item() / len(self.agent_set)
            train_info["g_l2_loss"] += l2_loss.item() / len(self.agent_set)
            return actor_loss
    ...
    def train(self, sp_buf, xp_buf, update_actor=True):
        ...
        for _ in range(self.ppo_epoch):
            ...
            if self._use_max_grad_norm:
                ...
                actor_grad_norm = get_gard_norm(self.policy.actor.parameters())
            gate_grad_norm =  nn.utils.clip_grad_norm_(
                    self.gate.parameters(), self.max_grad_norm
                )
            train_info["actor_grad_norm"] += actor_grad_norm.item()
            train_info["gate_grad_norm"] += gate_grad_norm.item()
            self.policy.actor_optimizer.step()
