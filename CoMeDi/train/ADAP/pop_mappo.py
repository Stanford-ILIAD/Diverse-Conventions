...
class Pop_MAPPO():
    ...
    def ppo_update(self, sample, multisample, update_actor=True):
        """
        ...
        full_loss = value_loss * self.value_loss_coef
        if update_actor:
            full_loss += policy_loss - dist_entropy * self.entropy_coef
        full_loss += multisample
        ...
        return value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, full_loss

    def train(self, buffers, update_actor=True):
        """
        ...
        for i in range(self.popsize):
            ...
            fulltraininfo.append(train_info)
            fulladvantages.append(advantages)

        for epch in range(self.ppo_epoch):
            print('EPOCH', epch)
            generators = []
            for i in range(self.popsize):
                buffer = buffers[i]
                advantages = fulladvantages[i]
                ...
                if self._use_recurrent_policy:
                    ...
                    data_generator = buffer.feed_forward_generator(
                        advantages, self.num_mini_batch)
                generators += [data_generator]

            for multisample in zip(*generators):
                sample_poploss = 0
                for cross_sample in multisample:
                    sample_poploss += self.pop_loss.get_population_loss(self.policies, cross_sample)
                ...
                for i in range(self.popsize):
                    ...
                    value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, full_loss \
                        = self.ppo_update(sample, sample_poploss, update_actor)
                    accum_loss += full_loss
        ...
        for train_info in fulltraininfo:
            for k in train_info.keys():
                train_info[k] /= num_updates

        return fulltraininfo

    def get_actions(self, *args):
        return self.policy.get_actions(*args)

    def get_values(self, *args):
        return self.policy.get_values(*args)

    def set_agent(self, agent_num):
        self.policy = self.policies[agent_num]
