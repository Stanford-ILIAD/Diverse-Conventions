...
def generate_player(args):
    ...
    run_dir = Path(os.path.split(os.path.dirname(os.path.abspath(__file__)))[0] + "/hanabi/results/" + (args.loss_type or "standard") + "/" + str(args.seed))
...
def main():
    ...
    pop_runner = XPlayer(args, players, xgym, xp_buffers, args.episode_length//10, 0.2)
    pop_runner.run()
